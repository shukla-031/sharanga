...
def generate_asset_number():
    """Generate unique asset number (SHAR-XXXX)"""
    conn = get_db_connection()
    cursor = conn.cursor()
...
def save_scan_data(asset_number, scan_data):
    """Save scan data to assets table"""
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    asset = cursor.execute('SELECT id FROM assets WHERE asset_number = ?', (asset_number,)).fetchone()
    if asset:
        cursor.execute('''
            INSERT INTO scan_history (asset_id, risk_score, suspicious_processes, total_apps, total_processes, full_data)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            asset['id'],
            stats.get('risk_score', 0),
